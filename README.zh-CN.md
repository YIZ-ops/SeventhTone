...
Seventh Tone News Reader 是一款面向英语学习者的跨平台新闻阅读应用。它聚合了来自 Sixth Tone 的优质英语文章，并通过 AI 练习、内置词典、句子摘录、生词本和详尽的阅读数据分析来增强阅读体验 —— 一切都封装在简洁的移动优先界面中，可同时部署为 Web 应用和 Android 原生应用。
...
- **阅读时长统计** — 自动记录每篇文章的阅读时长，切换页面时自动暂停。
...
- **选中高亮保存** — 在文章中选中任意句子，保存至句库并用 mark.js 高亮标记。
...
| 变量                | 位置             | 说明                                       |
| ------------------- | ---------------- | ------------------------------------------ |
| `GEMINI_API_KEY`    | `.env`           | 通过 Vite `define` 暴露给客户端的 API 密钥 |
| `LLM_CONFIG.apiKey` | `src/api/llm.ts` | SiliconFlow API 密钥，用于 AI 练习         |
