...
export default function BookmarkModal({ onClose, onSave }: Props) {
  ...
  return (
    <div data-popup className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm sm:p-4" onClick={() => { if (!justOpenedRef.current) onClose(); }}>
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-sm overflow-hidden shadow-2xl animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-0 sm:zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-4 pt-4 pb-safe-or-4 border-t border-gray-100 bg-gray-50 flex justify-end space-x-3">
          {isCreating && (
            <button
