...
export default function ArticleCard({ article }: Props) {
  ...
  return (
    <Link
      ...
      className="group block bg-white rounded-3xl border border-gray-100 overflow-hidden hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.08)] transition-all duration-500 active:scale-[0.99]"
    >
      <div className="flex flex-col">
        ...
        <div className="p-6 md:p-8">
          <div className="flex items-center space-x-2 mb-4">
            <span className="text-[10px] font-medium text-gray-400 uppercase tracking-widest">{timeAgo}</span>
          </div>
          <h3 className="text-xl md:text-2xl font-serif font-bold text-gray-900 leading-tight mb-3 group-hover:text-brand transition-colors duration-300">
            {article.name}
          </h3>
          <p className="text-sm md:text-base text-gray-500 line-clamp-2 mb-6 leading-relaxed font-serif italic">{article.summary}</p>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-gray-50">
            {article.userInfo ? (
              ...
              <span className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">Seventh Tone</span>
            )}
            <div className="w-8 h-8 rounded-full border border-gray-100 flex items-center justify-center group-hover:bg-brand group-hover:border-brand transition-all duration-300">
