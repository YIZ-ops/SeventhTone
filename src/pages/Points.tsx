    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-4xl mx-auto pb-32 space-y-6">
      <section className="overflow-hidden rounded-2xl border border-amber-100 bg-white px-5 py-5 shadow-sm dark:border-amber-500/20 dark:bg-slate-800">
        ...
        <div className="rounded-xl bg-amber-50/70 px-5 py-5 dark:bg-amber-500/10">
          ...
          <p className="mt-3 text-5xl font-bold text-gray-900 dark:text-gray-100">{summary.totalPoints}</p>
        </div>
      </section>
      ...
      {transactions.length === 0 ? (
        <div className="text-center py-32 bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-600 shadow-sm">
          ...
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">No points yet</h2>
              ...
              >
                <div className="space-y-2">
                  <div className="min-w-0 space-y-1.5">
                    ...
                    {item.articleTitle && <p className="text-xs leading-5 break-words text-gray-500 dark:text-gray-400">{item.articleTitle}</p>}
                  </div>
                  <div className="flex items-center justify-between gap-4">
                    <p className="text-xs text-gray-400 dark:text-gray-500">{formatDateTime(item.createdAt)}</p>
                    <p className="shrink-0 text-sm font-bold leading-none text-amber-600 dark:text-amber-300">+{item.points}</p>
                  </div>
                </div>
              </motion.div>
            );
